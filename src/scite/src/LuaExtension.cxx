...
bool LuaExtension::InitBuffer(int index) {
	...
	//if (index > maxBufferIndex)	- не работало в случае, когда открыты фвйл добавлялся  не в конец - проще запросить через экстендер
...
int LuaExtension::HildiAlarm(const char* msg, int flag, const GUI::gui_char *p1 = NULL, const GUI::gui_char *p2 = NULL, const GUI::gui_char *p3 = NULL) {
	int handled = NULL; 
	if (luaState) {
		lua_getglobal(luaState, "HildiAlarm");
		if (lua_isfunction(luaState, -1)) {
