...
static int sf_OrderTab(lua_State* L) {
	...
	while (lua_next(L, 1) != 0) {
		...
		/* удал€ем 'value'; сохран€€ 'key' дл€ следующей итерации */
		lua_pop(L, 1);
	}
...
bool LuaExtension::InitBuffer(int index) {
	...
	//if (index > maxBufferIndex)	- не работало в случае, когда открыты фвйл добавл€лс€  не в конец - проще запросить через экстендер
