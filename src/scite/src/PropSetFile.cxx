...
//!		importName += ".properties";
		...
		SString sPath = importName.c_str();
		ExpandAllInPlace(*this, sPath, 100);

		bool loaded = false; //!-add-[import]
		//FilePath importPath(directoryForImports, FilePath(GUI::StringFromUTF8(importName.c_str())));
		FilePath importPath(directoryForImports, FilePath(GUI::StringFromUTF8(sPath.c_str())));
...
bool PropSetFile::Read(const char *sciteDefaultHome, FilePath filename, FilePath directoryForImports,
	FilePath imports[], int sizeImports) {
	//Прижде всего необходимо установить путь к преложению - для импорта из сесссии и проч. Почему-то недостаточно сделать это снаружи
	props["SciteDefaultHome"] = sciteDefaultHome ;
	return Read(filename, directoryForImports,imports, sizeImports);		
}
