...
GUI::gui_string FilePath::GetFileName() const	{
	return fileName;
}
...
void FilePath::List(FilePathSet &directories, FilePathSet &files, const GUI::gui_char *fileTypes) {
	...
	//Сначала для каждой из масок добавим файлы, ей соответствующие
		const GUI::gui_char *patElement = pat.c_str() + start;
		wildCard.Set(*this, patElement);
		complete = false;
		hFind = ::FindFirstFileW(wildCard.AsInternal(), &findFileData);
	...
	//Потом добавимвсе директории
	wildCard.Set(*this, GUI_TEXT("*.*"));
	complete = false;
	hFind = ::FindFirstFileW(wildCard.AsInternal(), &findFileData);
	if (hFind != INVALID_HANDLE_VALUE) {
		while (!complete) {
