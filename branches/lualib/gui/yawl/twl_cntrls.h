...
public:
  ...
  // override
  void calc_size();
  virtual LRESULT WWndProc(UINT nMsg, WPARAM wParam, LPARAM lParam);

};
class EXPORT IOnEnter {
public:
	virtual void OnEnter() = 0;
};
class EXPORT CCmbItem: public CMWWnd{//Встроенный едитконтрол комбобокса
public:
	IOnEnter* m_pCmb;
	
	CCmbItem(){}

	void Connect(HWND hwnd, IOnEnter* pcmb)
	{ 
		m_pCmb = pcmb;
		m_hwnd = hwnd;
		Attach(hwnd);
...
class EXPORT TComboBox: public TControl, public IOnEnter {
public:
	TComboBox(TWin *parent, int id, bool is_edit=false, bool is_sorted=false);
	int height()  {return 30;}//!!!TODO!!! -  потом переделать по-честному
	virtual void OnEnter();
	void InsertString(int item,LPCSTR string); //Вставка строки, либо добавление в конец для сортируемогою При -2 - всавка в едитконтрол
	LPSTR GetString(int item); //При -2 вернем текст едитконтрола
	void DeleteString(int item);
	int FindString(const LPSTR string);
	int GetCount();
	void SetCurSel(int item);
	int GetCurSel();
	virtual LRESULT WWndProc(UINT nMsg, WPARAM wParam, LPARAM lParam);
	//virtual void get_rect(Rect &rt, bool use_client=false);
protected:
	CCmbItem m_edit;
	bool m_sorted;
};
class EXPORT TTextBox: public TControl, public IOnEnter{
public:
	TTextBox(TWin *parent, int id, bool is_numeric=false, bool is_capt=false);
	virtual void OnEnter();
	void SetText(LPCSTR string); 
	LPSTR GetText();
	virtual LRESULT	WWndProc(UINT nMsg, WPARAM wParam, LPARAM lParam);
private:
	LPSTR m_content;
...
public:
  ...
  LRESULT WWndProc(UINT nMsg, WPARAM wParam, LPARAM lParam);
  ...
  virtual void handle_oncreate() = 0;
