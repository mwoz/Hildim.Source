...
LRESULT	TCheckBox::WWndProc(UINT nMsg, WPARAM wParam, LPARAM lParam)
{

	return CallOldWP(nMsg,wParam,lParam);
}
...
TComboBox::TComboBox(TWin *parent, int id, bool is_edit,bool is_sorted)
 : TControl(parent,L"combobox",L"",id,
     CBS_DROPDOWN | CBS_AUTOHSCROLL | WS_VSCROLL | WS_BORDER | (is_sorted? CBS_SORT : 0)),m_sorted(is_sorted)
{
	COMBOBOXINFO cbi;
	...
	{
		m_edit.Connect(cbi.hwndItem, (IOnEnter*)this);
	}
	//cbi->hwndItem
}
void TComboBox::OnEnter()
{
}
void TComboBox::InsertString(int item,LPCSTR string) //Вставка строки, либо добавление в конец для сортируемогою При -2 - всавка в едитконтрол
{
	if(item == -2)
	{
		::SendMessageA(m_edit.m_hwnd,WM_SETTEXT,0,(LPARAM)string);
	}
	else if (m_sorted)
	{
		::SendMessageA(m_hwnd,CB_ADDSTRING,0,(LPARAM)string);
	}
	else
	{
		::SendMessageA(m_hwnd,CB_INSERTSTRING,item,(LPARAM)string);
	}
}
                                            
LPSTR TComboBox::GetString(int item) //При -2 вернем текст едитконтрола
{
	LPSTR string;
	if(item == -2)
	{
		int len = ::SendMessageA(m_edit.m_hwnd,WM_GETTEXTLENGTH,0,0);
		string = new CHAR[len+1];
		::SendMessageA(m_edit.m_hwnd,WM_GETTEXT,len + 1,(LPARAM)string);
	}
	else
	{
		int len = ::SendMessageA(m_hwnd,CB_GETLBTEXTLEN,item,0);
		string = new CHAR[len+1];
		::SendMessageA(m_hwnd,CB_GETLBTEXT,item,(LPARAM)string);
	}
	return string;
}
void TComboBox::DeleteString(int item)
{
	::SendMessageA(m_hwnd,CB_GETLBTEXT,item,0);
}
int TComboBox::FindString(const LPSTR string)
{
	return ::SendMessageA(m_hwnd,CB_FINDSTRING,-1,(LPARAM)string);
}
int TComboBox::GetCount()
{
	return ::SendMessageA(m_hwnd,CB_GETCOUNT,0,0);
}
void TComboBox::SetCurSel(int item)
{
	::SendMessageA(m_hwnd,CB_SETCURSEL,item,0);
}
int TComboBox::GetCurSel()
{
	return ::SendMessageA(m_hwnd,CB_GETCURSEL,0,0);
}
LRESULT	TComboBox::WWndProc(UINT nMsg, WPARAM wParam, LPARAM lParam)
{
	switch (nMsg)
	{
	case WM_COMMAND:
		{
			LRESULT l = CallOldWP(nMsg,wParam,lParam);
...
TTextBox::TTextBox(TWin *parent, int id, bool is_numeric, bool is_capt)
: TControl(parent,L"edit",L"",id,
  WS_BORDER | ES_AUTOHSCROLL | (is_numeric? ES_NUMBER :0) | (is_capt? ES_UPPERCASE :0))
{
		m_content = NULL;
		Attach(m_hwnd);
}
void TTextBox::OnEnter()
{
}
void TTextBox::SetText(LPCSTR string)
{
	::SendMessageA(m_hwnd,WM_SETTEXT,0,(LPARAM)string);
}
LPSTR TTextBox::GetText()
{
	int len = ::SendMessageA(m_hwnd,WM_GETTEXTLENGTH,0,0);
	LPSTR string = new CHAR[len+1];
	::SendMessageA(m_hwnd,WM_GETTEXT,len + 1,(LPARAM)string);
	return string;
}
LRESULT	TTextBox::WWndProc(UINT nMsg, WPARAM wParam, LPARAM lParam)
{
	switch (nMsg)
	{
	//case WM_KEYUP:
	//	if(wParam == 13) OnEnter();
	//	break;
	case WM_SETFOCUS:
		if(m_content) delete m_content;
		m_content = GetText();
	case WM_KILLFOCUS:
		if (strcmp(m_content,GetText())) OnEnter();
	}
	return CallOldWP(nMsg,wParam,lParam);
}
   ...
   ), m_file_name(NULL)
{
	...
	send_msg(EM_SETEVENTMASK, 0, ENM_KEYEVENTS|ENM_CHANGE);
}

LRESULT	TMemo::WWndProc(UINT nMsg, WPARAM wParam, LPARAM lParam)
{
	switch(nMsg){
	case WM_SETFOCUS:
		handle_onfocus(true);
	...
	case WM_CREATE:
		...
		break;
	case WM_KEYDOWN:
		if(handle_onkey(wParam)) return true;
		break;
	case WM_CHAR:
		if(wParam==9)return true;
		break;
	}
	return CallOldWP(nMsg,wParam,lParam);
}
