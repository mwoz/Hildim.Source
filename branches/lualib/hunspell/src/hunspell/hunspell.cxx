...
int Hunspell::spell(const char * word, int * info, char ** root, int * captype)
{
  ...
  int captype2 = 0;
  if (!captype) captype = &captype2; else *captype = 0;
  int abbv = 0;
  int wl = 0;
...
int Hunspell::add_with_affix(const char * word, const char * example, char ** out_flags)
{
	if (pHMgr[0]) return (pHMgr[0])->add_with_affix(word, example, out_flags);
	return 0;
}
