...
BOOL SciTEWin::FindMessage(HWND hDlg, UINT message, WPARAM wParam) {
	...
	case WM_COMMAND:
		if (ControlIDOfCommand(wParam) == IDCANCEL) {
			...
			return FALSE;
					...
					(ControlIDOfCommand(wParam) == IDFINDALL)) {
			dlg.GrabFields();
			FillCombos(dlg);
			if (closeFind) {
				rFindReplace = wFindReplace.GetPosition();
			...
			if(ControlIDOfCommand(wParam) == IDFINDALL)
			{
				if(!heightOutput) ToggleOutputVisible();
				FindAll();
			}
			else
			{
	...
	//!			FindNext(reverseFind);
				...
				if (!closeFind) MoveFindRepFromText();
			}
			return TRUE;
...
void SciTEWin::Find() {
	if (wFindReplace.Created()) {
		...
		{
			//Если при открытом поиске нажали замену - закроем старый диалог
			//!ТУДУ!Проверить, что будет при поиске в ниженй панельке
			rFindReplace = wFindReplace.GetPosition();
			::EndDialog(hDlg, IDOK);
			wFindReplace.Destroy();
		}
...
BOOL SciTEWin::GrepMessage(HWND hDlg, UINT message, WPARAM wParam) {
	...
	case WM_COMMAND:
		if (ControlIDOfCommand(wParam) == IDCANCEL) {
			rFindInFiles = wFindInFiles.GetPosition();
			::EndDialog(hDlg, IDCANCEL);
...
void SciTEWin::FindInFiles() {
	...
	wFindInFiles.MoveToLeftTop(rFindInFiles);
	wFindInFiles.Show();
}
...
void SciTEWin::Replace() {
	if (wFindReplace.Created()) {
		HWND hDlg = reinterpret_cast<HWND>(wFindReplace.GetID());
		if (replacing) {
			int l = SelectionIntoFind(false);
			Dialog dlg(hDlg);
			dlg.SetItemTextU(IDFINDWHAT, findWhat);
			::SetFocus(hDlg);
			::SetFocus(::GetDlgItem(hDlg, l == 0 ? IDFINDWHAT : IDREPLACEWITH));
			return;
		}
		else
		{
	...
	case WM_COMMAND:
		if (ControlIDOfCommand(wParam) == IDCANCEL) {
			...
			if (bHasLine || bHasChar) {
				...
				if (extender) extender->OnNavigation("Line");
