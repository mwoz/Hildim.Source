...
bool SciTEWin::ModelessHandler(MSG *pmsg) {
	if (wFindReplace.GetID() || wFindInFiles.GetID() || wParameters.GetID()) {
		//ѕропускаем в основное окно все клавиатурные команды
...
{//ќтодвигаем  диалог поиска замены с текущей позиции
...
void SciTEWin::PerformGrep() {
	...
	if (findCommand == "") {
		...
		//Ќепон€тно, где устанавливаетс€ find.in.binary - но если установлено - не будем использовать регул€рный поиск
		searchParams.append(props.GetInt("find.in.binary") ? "b" : regExp ? "r" : "~");
		searchParams.append(subDirSearch ? "s" : "~");
		searchParams.append("~"); //признак группировки по файлам выставл€етс€ только из луа дилога 
		searchParams.append("\0", 1);
