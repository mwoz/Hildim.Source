...
void dispatch_vref(lua_State* L,int idx)
{
	if (idx != 0) {
...
int dispatch_rvref(lua_State* L,int idx)
{
	if (idx != 0) {
		lua_rawgeti(L,LUA_REGISTRYINDEX,idx);		
...
public:
	...
	{}
	bool PanelWindow::command(int iId, int iCmd) 
	{ 
		LuaCmd * pl;
		TControl * pc;
		TWin* w;
		start_child();
		while (next_child(w)) {
			if((pc = dynamic_cast<TControl*>(w)) && pc->m_id == iId)
			{
				if((pl = dynamic_cast<LuaCmd*>(w)) != NULL)
				{ 
					pl->OnCmd(iCmd);
					break;
				}
...
class FrameWindow: public LuaWindow
{
public:
	FrameWindow(lua_State* l)
		: LuaWindow(L"",l,get_parent(),0,true)
	{}
};
class PaletteWindow;
...
public:
	...
	ContainerWindow(const wchar_t* caption, lua_State* l)
		: PaletteWindow(caption,l,WS_EX_TOOLWINDOW|WS_EX_TOPMOST,WS_POPUP|WS_SIZEBOX|WS_CAPTION),select_name(NULL),double_name(NULL)
	{
...
public:
	LuaControl(lua_State *l)
		...
		, m_hpopup_menu(0), onclose_idx(0), onfocus_idx(0), click_idx(0), change_idx(0), create_idx(0)
	{}
	...
	virtual void set_on_click(int iarg)
	{
		function_ref(L,2,&click_idx);
	}
	...
	virtual void set_change(int iarg)
	{
		function_ref(L,iarg,&change_idx);
	}
	virtual void set_create(int iarg)
	{
		function_ref(L,iarg,&create_idx);
	}

};

class TMemoLua: public TMemo, public LuaControl, public LuaCmd
{
public:
	...
	virtual void handle_oncreate()
	{
		dispatch_vref(L,onfocus_idx);
	}
	virtual bool OnCmd(int cmd)
	{
		if(cmd == EN_CHANGE)
		{
			return dispatch_rvref(L,change_idx);
		}
		return 0;
	}
};
...
{
public:
	TComboBoxLua(TWin *parent, lua_State *l, int id,bool is_edit = false, bool is_sorted = false)
		:TComboBox(parent,id,is_edit, is_sorted),LuaControl(l)
	{}
	virtual void TComboBoxLua::OnEnter()
	{
		dispatch_vref(L,click_idx);
	}
};

class TCheckBoxLua: public TCheckBox, public LuaControl, public LuaCmd
{
public:
	TCheckBoxLua(TWin *parent, lua_State *l, LPCSTR caption, int id, bool is_auto = true)
		:TCheckBox(parent,L"",id,is_auto),LuaControl(l)
	{
		SendMessageA(m_hwnd,WM_SETTEXT,0,(LPARAM)caption);
	}

	bool TCheckBoxLua::OnCmd(int cmd)
	{
		if(cmd == BN_CLICKED)
		{
			return dispatch_rref(L,click_idx, 2);
		}
		return 0;
	}
};
class TButtonLua: public TButton, public LuaControl, public LuaCmd
{
public:
	TButtonLua(TWin *parent, lua_State *l, LPCSTR caption, int id, long style = PUSHBUTTON)
		:TButton(parent,L"",id,style),LuaControl(l)
	{
		SendMessageA(m_hwnd,WM_SETTEXT,0,(LPARAM)caption);
	}
	virtual bool TButtonLua::OnCmd(int cmd)
	{
		if(cmd == BN_CLICKED)
		{
			return dispatch_rref(L,click_idx, 2);
...
{
public:
	TLabelLua(TWin *parent, lua_State *l, LPCSTR caption, int id)
		:TLabel(parent,L"", id),LuaControl(l)
	{
		SendMessageA(m_hwnd,WM_SETTEXT,0,(LPARAM)caption);
	}

};
class TTextBoxLua: public TTextBox, public LuaControl
{
public:
	TTextBoxLua(TWin *parent, lua_State *l, int id)
		:TTextBox(parent,id, false,false),LuaControl(l)
	{}
	virtual void TTextBoxLua::OnEnter()
	{
		dispatch_vref(L,click_idx);
	}
};
...
{
public:
	TTreeViewLua(TWin *parent, lua_State *l)
		: TTreeView((TEventWindow*)parent,true,false),
		LuaControl(l)
	{
		//if (! multiple_columns) {
		//	add_column(L"*",200);
	...
	void add_item(int parrent,const wchar_t* caption,bool hasChild,void *pData)
	{
		if(parrent == 0)
		{
			parrent = (int)GetSelection();
		}
		add((Handle)parrent, caption,0,-1,hasChild,pData);
...
int do_open_dlg(lua_State* L)
{
	...
	const wchar_t* path = StringFromUTF8(luaL_optstring(L,3,""));
	TOpenFile tof (get_parent(),caption,filter,path);
	if (tof.go()) {
...
int new_panel(lua_State* L)
{
	...
	((TWin*)pw)->align(alLeft,luaL_checkinteger(L,1));
	s_last_parent = pw;
	return wrap_window(L,pw);
}
int new_frame(lua_State* L)
{/////////////////////////
	PanelWindow* pw = new PanelWindow(L);
	int h = luaL_checkinteger(L,1);
	((TWin*)pw)->align(alTop,h);
	pw->enable_resize(false,-1,luaL_checkinteger(L,1));
	s_last_parent = pw;
	return wrap_window(L,pw);
}
...
{
	TMemoLua *m = new TMemoLua(get_last_parent(),L, i_winId++);
	return wrap_window(L,m);
}
int new_checkbox(lua_State* L)
{
	TCheckBoxLua *m = new TCheckBoxLua(get_last_parent(),L,luaL_checkstring(L,1), i_winId++);
	return wrap_window(L,m);
}
int new_button(lua_State* L)
{
	TButtonLua *m = new TButtonLua(get_last_parent(),L,luaL_checkstring(L,1), i_winId++);
	return wrap_window(L,m);
}
int new_label(lua_State* L)
{
	TLabelLua *m = new TLabelLua(get_last_parent(),L,luaL_checkstring(L,1), i_winId++);
	return wrap_window(L,m);
}
int new_textbox(lua_State* L)
{
	TTextBoxLua *m = new TTextBoxLua(get_last_parent(),L,i_winId++);
	return wrap_window(L,m);
}
int new_combobox(lua_State* L)
{
	TComboBoxLua *m = new TComboBoxLua(get_last_parent(),L, i_winId++, optboolean(L,1,false),optboolean(L,2,false));
	return wrap_window(L,m);
}
...
int window_settext(lua_State* L)
{
	TWin* w = window_arg(L);
	if(TMemoLua *m = dynamic_cast<TMemoLua*>(w))
	{
		m->set_text(StringFromUTF8(luaL_checkstring(L,2)));
		m->go_to_end();
	}
	else if(TTextBox* tb = dynamic_cast<TTextBox*>(w))
	{
		tb->SetText(luaL_checkstring(L,2));
	}
	return 0;

}

int window_gettext(lua_State* L)
{
	TWin* w = window_arg(L);
	if(TMemoLua *m = dynamic_cast<TMemoLua*>(w))
	{
...
int window_aux_item(lua_State* L, bool at_index)
{
	...
	else if(TComboBoxLua* cb = dynamic_cast<TComboBoxLua*>(w))
	{
		cb->InsertString(luaL_checkinteger(L,2),luaL_checkstring(L,3));
	}else if (TTreeViewLua* tv = dynamic_cast<TTreeViewLua*>(w))
	{
		tv->add_item(luaL_checkinteger(L,2),StringFromUTF8(luaL_checkstring(L,3)),luaL_checkinteger(L,4),(void*)luaL_ref(L,5));
	}
	return 0;
}
...
int window_get_item_text(lua_State* L)
{
	...
	{
		LPSTR buff = cb->GetString(luaL_checkinteger(L,2));
		lua_pushstring(L,buff);
		delete[] buff;		
	}
	else return 0;
...
int window_select_item(lua_State* L)
{
	...
	{
		cb->SetCurSel(luaL_checkinteger(L,2));	
	}
	else if(TTabControlLua* tc = dynamic_cast<TTabControlLua*>(w))
	{
		::SendMessage(tc->m_hwnd,4876, (WPARAM)luaL_checkinteger(L,2)-1, 0);//TCM_SETCURSEL
		tc->handle_select(luaL_checkinteger(L,2)-1);
	}
	return 0;
}
...
int window_get_checked(lua_State* L)
{
	TCheckBoxLua* lc;
	if(lc = dynamic_cast<TCheckBoxLua*>(window_arg(L,1)))
	{
		lua_pushboolean(L, lc->check());		
		return 1;
	}
	return 0;
}
int window_check(lua_State* L)
{
	TCheckBoxLua* lc = dynamic_cast<TCheckBoxLua*>(window_arg(L,1));
	lc->check(luaL_checkinteger(L,2) != 0);
	return 0;
}
//int window_settext(lua_State* L)
//{
...
int window_on_click(lua_State* L)
{
	TWin* w = window_arg(L);
	LuaControl* b = dynamic_cast<LuaControl*>(w);
	b->set_on_click(2);
	//if(TButtonLua* b = dynamic_cast<TButtonLua*>(w)) {
	//	b->set_on_click(2);
	//} else if (TCheckBoxLua* ch = dynamic_cast<TCheckBoxLua*>(w)) {
	//	ch->set_on_click(2);
	//} 
	//else if (TTextBoxLua* tb = dynamic_cast<TTextBoxLua*>(w))
	//{
	//	tb->set_on_click(2);
	//}
	//else if (TComboBoxLua* cb = dynamic_cast<TComboBoxLua*>(w))
...
int window_on_change(lua_State* L)
{
	TWin* w = window_arg(L);
	TMemoLua* m = dynamic_cast<TMemoLua*>(w); 
	m->set_change(2);
	return 0;
}
int window_on_create(lua_State* L)
{
	TWin* w = window_arg(L);
	LuaControl* l = dynamic_cast<LuaControl*>(w); 
	l->set_create(2);
	return 0;
}
...
static int append_file(lua_State *L, int idx, _wfinddata_t c_file, bool look_for_dir, bool with_time)
{
	if (((c_file.attrib & _A_SUBDIR) != 0) == look_for_dir) {
		if (look_for_dir && (EQ(c_file.name, L".") || EQ(c_file.name, L".."))) return idx;
		lua_pushinteger(L,idx);
		if (with_time){
			lua_newtable(L);
			lua_pushinteger(L, 1);
			lua_pushstring(L, UTF8FromString(c_file.name));
			lua_settable(L, -3);
			lua_pushinteger(L, 2);
			lua_pushnumber(L, c_file.time_write);
			lua_settable(L, -3);
		}
		else{
			lua_pushstring(L, UTF8FromString(c_file.name));	
...
static int do_files(lua_State *L)
{
	...
	bool with_time = optboolean(L, 3, false);
	long hFile = _wfindfirst(mask,&c_file);
	int i = 1;
	if (hFile == -1L) { lua_pushboolean(L,0); return 1; }
	lua_newtable(L);
	i = append_file(L, i, c_file, look_for_dir, with_time);
...
static const struct luaL_reg window_methods[] = {
	...
	{"get_checked",window_get_checked},
