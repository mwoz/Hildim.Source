...
SciTEWin::SciTEWin(Extension *ext) : SciTEBase(ext) {
	...
	//Выставим место по умолчанию для поиска.замены
...
//		winPlace.showCmd = SW_HIDE; //Пока окно не показываем, только позиционируем - чтобы избежать фликеринга, если открыт другой инстанс, а наш потом закроется. Покажем в Run позднее
//		::SetWindowPlacement(MainHWND(), &winPlace);
//	}
//}
	...
	while (going) {
		...
		if (going) {
			if (!ModelessHandler(&msg)) {
				hw = msg.hwnd; //Проверяя перед TranslateAccelerator на несовпадения hwnd мессаджа с окнами scite, избегаем его выполнения для окон боковой панели
				if (!GetID() ||(reinterpret_cast<HWND>(wEditor.GetID())!=hw&&reinterpret_cast<HWND>(wOutput.GetID())!=hw&&reinterpret_cast<HWND>(GetID())!=hw) ||
