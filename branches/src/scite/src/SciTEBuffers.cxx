...
//!			        "error.marker.back", ColourRGB(0xff, 0xff, 0)));
			...
			if (style == SCE_ERR_GCC){
				...
				while (wOutput.Call(SCI_GETSTYLEAT, startPosLine + posInFind) == SCE_ERR_GCC && lineLength > posInFind) posInFind++;
				while (wOutput.Call(SCI_GETSTYLEAT, startPosLine + posInFind + posInLine) != SCE_ERR_FIND_VALUE && lineLength > posInFind + posInLine) posInLine++;
				while (wOutput.Call(SCI_GETSTYLEAT, startPosLine + posInFind + posInLine + lenSelected) == SCE_ERR_FIND_VALUE && lineLength > posInFind + posInLine + lenSelected) lenSelected++;
