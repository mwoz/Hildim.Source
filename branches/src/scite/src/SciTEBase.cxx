...
{//Находим все вхождения и выводим на консоль
	int len = wOutput.Call(SCI_GETLENGTH);
	wOutput.Call(SCI_SETSEL,len,len);
	int ancCurrent = wEditor.Call(SCI_GETANCHOR);
	int posCurrent = wEditor.Call(SCI_GETCURRENTPOS);
	int iLn = wOutput.Call(SCI_GETLINECOUNT);
	...
	//wOutput.SendPointer(SCI_APPENDTEXT,s.length(),(void*)s.c_str());
	int iOcur = 0;
	int iLines = 0;
	if (posFirstFound != -1) {
		int posFound = posFirstFound;
	...
	wOutput.CallString(SCI_REPLACESEL, 0, s.c_str());
	//wOutput.Call(SCI_SETSEL, 1, 1);
	wOutput.Call(SCI_SETFIRSTVISIBLELINE, 0);
	...
	//wOutput.Call(SCI_SETFIRSTVISIBLELINE, iLn-1);
...
//!-end-[InsertAbbreviation]
	...
	//Переведем строку в нижний регистр - в результате вообще не сможем читать записанные в верхнем аббревиатуры, 
	//но записанные в нижнем будут прочитаны всегда
	_strlwr_s(linebuf,currentPos + 2);
...
void SciTEBase::CollapseOutput()	{
	wOutput.Call(SCI_COLOURISE, 0, -1);
	int maxLine = wOutput.Call(SCI_GETLINECOUNT);
	for (int line = 0; line < maxLine; line++) {
		int level = wOutput.Call(SCI_GETFOLDLEVEL, line);
		if ((level & SC_FOLDLEVELHEADERFLAG) &&
			(SC_FOLDLEVELBASE == (level & SC_FOLDLEVELNUMBERMASK))) {
				int lineMaxSubord = wOutput.Call(SCI_GETLASTCHILD, line, -1);
				wOutput.Call(SCI_SETFOLDEXPANDED, line, 0);
				if (lineMaxSubord > line)
					wOutput.Call(SCI_HIDELINES, line + 1, lineMaxSubord);
		}
	}
}
...
//!-end-[SetBookmark]
				if (notification->margin == 2) {
					if(notification->nmhdr.idFrom == IDM_RUNWIN)
					{
						int lineClick = wOutput.Call(SCI_LINEFROMPOSITION, notification->position);
						if ((notification->modifiers & SCMOD_SHIFT) && (notification->modifiers & SCMOD_CTRL)) {
							FoldAll();
						} else {
							int levelClick = wOutput.Call(SCI_GETFOLDLEVEL, lineClick);
							if (levelClick & SC_FOLDLEVELHEADERFLAG) {
								if (notification->modifiers & SCMOD_SHIFT) {
									...
									// Toggle this line
									wOutput.Call(SCI_TOGGLEFOLD, lineClick);
								}
							}
						}
					}
...
void SciTEBase::CombineContextMenues(const char *mnuMain, SString &mnuSlave) {
	if (!mnuMain)
		return;
	SString m = mnuMain;
...
void SciTEBase::ContextMenu(GUI::ScintillaWindow &wSource, GUI::Point pt, GUI::Window wCmd) {
	...
	if (wSource.GetID() == wOutput.GetID()) {
		...
		CombineContextMenues(extender->OnContextMenu(0, 0, "OUTPUT"), userContextMenu);
		userContextMenu.substitute('|', '\0');
		const char *userContextItem = userContextMenu.c_str();
		const char *endDefinition = userContextItem + userContextMenu.length();
		...
		CombineContextMenues(extender->OnContextMenu(0, 0, "EDITOR"), userContextMenu);
		userContextMenu.substitute('|', '\0');
		const char *userContextItem = userContextMenu.c_str();
		const char *endDefinition = userContextItem + userContextMenu.length();
		GenerateMenu(subMenu, userContextItem, endDefinition, item, isAdded);
