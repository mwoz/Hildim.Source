...
{//ѕри инициализированной luaState использует поиск луа дл€ определени€ индекса первого вхождени€ подстроки в текст
...
void SciTEBase::InternalGrep(GrepFlags gf, const GUI::gui_char *directory, const GUI::gui_char *fileTypes, const char *search) {
	...
	SString os = "";
	if (!(gf & grepStdOut)) {
		os.append(">??Internal search for \"").append(searchString.c_str()).append("\" in \"");
	...
	wOutput.Send(SCI_SETSEL, 0, 0);
	wOutput.Send(SCI_SETFIRSTVISIBLELINE, 0);
	wOutput.Send(SCI_REPLACESEL, 0, reinterpret_cast<sptr_t>("Wait...\n"));
	GrepOut grepOut;
	grepOut.iLines = 0;
	grepOut.iFiles = 0;
	grepOut.iInFiles = 0;
	...
	wOutput.Send(SCI_SETSEL, 0, 8);
	wOutput.Send(SCI_SETFIRSTVISIBLELINE, 0);
	wOutput.Send(SCI_COLOURISE, 0, -1);
	int maxLine = wOutput.Send(SCI_GETLINECOUNT);
	for (int line = 0; line < maxLine; line++) {
		int level = wOutput.Send(SCI_GETFOLDLEVEL, line);
		if ((level & SC_FOLDLEVELHEADERFLAG) &&
			(SC_FOLDLEVELBASE == (level & SC_FOLDLEVELNUMBERMASK))) {
				int lineMaxSubord = wOutput.Send(SCI_GETLASTCHILD, line, -1);
				wOutput.Send(SCI_SETFOLDEXPANDED, line, 0);
				if (lineMaxSubord > line)
					wOutput.Send(SCI_HIDELINES, line + 1, lineMaxSubord);
		}
	}
	wOutput.Send(SCI_SETSEL, 0, 8);
	wOutput.Send(SCI_REPLACESEL, 0, reinterpret_cast<sptr_t>(os.c_str()));
	wOutput.Send(SCI_SETSEL, 0, 0);
	wOutput.Send(SCI_SETFIRSTVISIBLELINE, 0);
}
