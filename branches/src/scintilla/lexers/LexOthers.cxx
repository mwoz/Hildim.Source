...
static int RecogniseErrorListLine(const char *lineBuffer, unsigned int lengthLine, int &startValue) {
	if (lineBuffer[0] == '>') {
		...
		if(strstr(lineBuffer,">??") == lineBuffer) return SCE_ERR_FIND_LIST;
		return SCE_ERR_CMD;
	} else if (lineBuffer[0] == '<') {
...
//!-end-[FindResultListStyle]
	...
	if(style == SCE_ERR_CMD || style == SCE_ERR_FIND_LIST) 
	{
		isFindList = false;
		findValue[0] = '\0';
	}
...
//!-start-[FindResultListStyle]
		if (valueSeparate && style == SCE_ERR_FIND_LIST) {
...
static void FoldErrorListDoc(unsigned int startPos, int length, int, WordList *[], Accessor &styler) {
	bool foldCompact = styler.GetPropertyInt("fold.compact", 1) != 0;
...
LexerModule lmErrorList(SCLEX_ERRORLIST, ColouriseErrorListDoc, "errorlist", FoldErrorListDoc, emptyWordListDesc);
