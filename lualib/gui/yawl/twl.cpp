...
POINT TEventWindow::fixed_size(bool isMin)
{
	if( m_fixed_size.x < 0)
	{
		POINT p;
...
{
	if(!m_do_resize&&(m_fixed_size.x<0)&&(height()!=m_fixed_size.y)&&m_fixed_size.y>0)
	{
		resize(width(),m_fixed_size.y);
	}
  ...
  switch (msg)
  {
    ...
    case WM_GETMINMAXINFO:
		if (This && This->cant_resize()) {
		  ...
		  pSizeInfo->ptMaxTrackSize = This->fixed_size(false);
		  pSizeInfo->ptMinTrackSize = This->fixed_size(true);
		}
		return 0;
