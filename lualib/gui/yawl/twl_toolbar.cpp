...
TListViewB::TListViewB(TWin* form, bool large_icons, bool multiple_columns, bool single_select)
{
	...
	::PostMessage(m_hwnd, LV_INIT, 0, 0);
}
...
LRESULT	TListViewB::WWndProc(UINT nMsg, WPARAM wParam, LPARAM lParam)
{
	switch(nMsg){
	case LV_INIT:
		handle_oncreate();
...
Handle TTreeView::GetSelection()
{
	return TreeView_GetSelection(this->m_hwnd);
}
