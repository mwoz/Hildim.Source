...
static int cur_shut(lua_State *L, cur_data *cur)
{
	...
	cur->closed = 1;
	SQLRETURN e = (SQLCloseCursor(cur->stmt->hstmt));
	if (error(e) && e != SQL_NULL_DATA) {
...
static int cur_more(lua_State* L) {
	cur_data* cur = getcursor(L, 1);
	SQLHSTMT hstmt = cur->stmt->hstmt;
	int ret;
	SQLSMALLINT numcols;
	...
	if (SQLMoreResults(hstmt) == SQL_SUCCESS) {
		...
		if (numcols > 0) {
			...
			//return  create_cursor(L, -1, cur->stmt, numcols);
			luaL_unref(L, LUA_REGISTRYINDEX, cur->colnames);
			luaL_unref(L, LUA_REGISTRYINDEX, cur->coltypes);
